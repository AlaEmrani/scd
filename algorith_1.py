from generate_dataset import generate_reference_models, base_differential_network
import numpy as np
import random
...
def get_lambda_hit(indices, cols_kron, val1, val2, v, lambda_t):
    ...
    # temp1 = np.zeros((cols_kron.shape[0], 2))
    # temp2 = np.zeros((cols_kron.shape[0], 2))
    # temp3 = np.zeros((cols_kron.shape[0], 2))
    ...
    # temp1[indices] = numerator
    # temp2[indices] = denom1
    # temp3[indices] = denom2
    # print(temp1[np.])
...
def get_kron_min_eigenvalue(mat1, mat2):
    eigenvalues1 = np.linalg.eigvals(mat1)
    eigenvalues2 = np.linalg.eigvals(mat2)

    eigenvalues_kron = np.outer(eigenvalues1, eigenvalues2).flatten()
    return np.min(eigenvalues_kron)


def get_lambda_star(sigma, sigma_hat, p1, p2):
    delta = np.abs(p1 - p2)
    delta_min = delta[delta > 0].min()
    eigenvalue_min = get_kron_min_eigenvalue(sigma, sigma_hat)
    s = len(np.where(delta != 0)[0])
    return delta_min * eigenvalue_min / (8 * s**.5)
...
dataset_1, precision_1, cov_1, dataset_2, precision_2, cov_2 = base_differential_network(100, 1000, decay_ratio=.1,
                                                                                         alternative_value=.8,
                                                                                         alternate_diag_index=1.1)
# dataset_1, precision_1, cov_1, dataset_2, precision_2, cov_2 = generate_reference_models(100, 1000, 10, mult=10)
# delta_star = precision_2 - precision_1
# delta_star_min = np.abs(delta_star).min()
sample_cov_1 = np.cov(dataset_1.T)
sample_cov_2 = np.cov(dataset_2.T)
print(sample_cov_1.shape)

real_cov1 = np.linalg.inv(precision_1)
real_cov2 = np.linalg.inv(precision_2)

print(len(np.where((precision_1 - precision_2) != 0)[0]))
print('lambda star: ', get_lambda_star(real_cov1, real_cov2, precision_1, precision_2))
# exit()

flatten_delta = (precision_1 - precision_2).flatten('F').reshape(-1, 1)
delta_index = np.array(range(len(flatten_delta))).reshape(-1, 1)
# print(np.concatenate((flatten_delta, delta_index), axis=1))
...
for i in range(50):
    new_lam, new_a_set_sa = alg_1(sample_cov_1, sample_cov_2, a_set_sa, lam)
    if new_lam <= 0:
        break
    a_set_sa, lam = new_a_set_sa, new_lam
    print(f'iter {i}: ', len(a_set_sa), lam)
